...
local settingsTable = {
	General = {
		...
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'Cherry Keybind'},
		-- buildwarnings
		-- rayfieldprompts
	...
	System = {
		...
		autoUpdate = {Type = "toggle", value = false, Name = "Auto Update"},
	}
}
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			...
			if useStudio then
				file = [[
		{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"Cherry Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Cherry Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RayfieldLibrary:Notify({Title = "Cherry minimisé", Content = "L'interface a été minimiser, clique sur 'Show Cherry' Pour l'ouvrir.", Duration = 7, Image = 4400697855})
		else
			RayfieldLibrary:Notify({Title = "Cherry minimisé", Content = `L'interface a été minimiser, clique sur 'Show Cherry' Pour l'ouvrir ou sinon sur cette lettre {settingsTable.General.rayfieldOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Cherry Settings', 0, true)

	if TabList['Cherry Settings'] then
		TabList['Cherry Settings'].LayoutOrder = 1000
	end

	if Elements['Cherry Settings'] then
		Elements['Cherry Settings'].LayoutOrder = 1000
	end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if Settings.LoadingTitle ~= "Rayfield Interface Suite" then
		LoadingFrame.Version.Text = "Cherry UI"
	end
	...
	if not Settings.DisableCherryPrompts then
		task.spawn(function()
			while true do
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Cherry Settings'])
		end)
	end)
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RayfieldLibrary:Notify({Title = "Cherry Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RayfieldLibrary:Notify({Title = "Cherry Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Cherry Configurations Error | '..tostring(result))
			RayfieldLibrary:Notify({Title = "Cherry Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
